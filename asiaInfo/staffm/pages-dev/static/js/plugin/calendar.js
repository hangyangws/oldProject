...
(function(win, undefined) {
    ...
    // 密保Date的getMonth方法
    Date.prototype.getMonthLong = Date.prototype.getMonthLong || function() {
    ...
    function Calendar($parent, timeList) {
        ...
        this.now = {
            ...
            month: nowDate.getMonthLong(), // 当前月（getMonth函数返回的是0~11，所以需要加一）
    ...
    Calendar.prototype.render = function() { // 渲染日历
        ...
        while (_length++ < _this.nowRender.dayLength) {
            // 判断当前日期时候需要高亮显示
            if (inDateList(_this.nowRender.year + '-' + _this.nowRender.month + '-' + (_length < 10 ? '0' + _length : _length), _this.timeList)) {
                _cal_body.push('<td class="hy-cal-use" data-num="' + _length + '"></td>');
            } else {
                _cal_body.push('<td data-num="' + _length + '"></td>');
            }
    ...
    Calendar.prototype.prevMonth = function() { // 上一月
        if (this.nowRender.month == 1) {
            ...
            if (--this.nowRender.month < 10) {
                this.nowRender.month = '0' + this.nowRender.month;
            }
        }
        this.render();
    };
