 ...
 * 用法：<input type="number" name="tel" data-vname="性别" data-vtype="require tel" />
 */
! function(win, $, undefined) {
    ...
    Verify.prototype = {
        reg: {
            ...
            name: /^[\u4e00-\u9fa5·]{2,20}$/, // 姓名
            positivenumber: /^0(\.\d+)?$|^[1-9]+0*[1-9]*0*(\.\d+)?$/ // 大于等于0的所有数
        },
                        ...
                        if (_val_str) {
                            ...
                            if (_ipt.vType.length > 1) {
                                ...
                                if (this.reg[_ipt.vType[_index_obj[_require_index]]].test(_val)) {
                                    ...
                                    // 提示错误信息
                                    !_param.noRemind && layer.open({ content: '请输入正确的' + _ipt.vName });
                                    return;
                                }
                            } else {
                                // 数据填充
                            ...
                            // 提示错误信息
                            !_param.noRemind && layer.open({ content: _ipt.vName + '不能为空' });
                            return;
                        }
                    } else {
                        // 是否有值
