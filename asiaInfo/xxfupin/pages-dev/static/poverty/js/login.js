/**
 * Created by hangyangws(hangyangws@foxmail.com) in 2016-12-06.
 */
...
! function(win, $, undefined) {
    ...
    var $passLogin = $('#passLogin'),
        $codeWrap = $('#codeWrap'),
        $passWrap = $('#passWrap'),
        $passIpt = $passWrap.find('.j-pass'),
        $getCode = $codeWrap.find('.j-get-code'),
        _is_code = true, // 时候可以发送验证码
        _reg_data = {
            type: /[^\#]\#{1}passLogin$/.test(win.location.href) ? 0 : 1, // 登录类型
        }, // ==>
        /*{
            mobile: '',
            vcode: ''
    ...
    $passWrap
        .on(win.method, '.j-see-pass', Project.seePass); // 查看密码
    // -----------------------
    // 本地模式，获取验证码（还没有开通手机）
