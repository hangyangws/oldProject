...
! function($) {
    ...
    var Pj = (function() {
        ...
        return {
             ...
             */
            checkNav: function() {
                $('.roll-nav-wrap').each(function() {
                    $(this).children().length <= $(this).data('min') && (
                        // 移除nav
                        $(this).closest('.item-nav').find('.roll-nav').addClass('none').prev().css('padding-bottom', '100px').end().remove()
                    );
                });
            },
